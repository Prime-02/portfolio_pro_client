// portfolio-builder/components/sections/testimonials/editor-components/TestimonialsLayoutTab.tsx

import { TestimonialsData, TestimonialsLayout } from "@/portfolio-builder/types/testimonials";
...
interface TestimonialsLayoutTabProps {
...
const SINGLE_COLUMN_LAYOUTS: TestimonialsLayout[] = ["list", "marquee"];

export default function TestimonialsLayoutTab({ data, onChange }: TestimonialsLayoutTabProps) {
    ...
    <div className="flex flex-col gap-5">
      <div className={sectionClass}>
        ...
        <div className="space-y-3">
          ...
          <input type="text" value={data.headline || ""} onChange={(e) => onChange("headline", e.target.value || undefined)}
            placeholder="What People Say" className="w-full bg-[var(--pb-input-bg)] border border-[var(--pb-input-border)] rounded-lg px-3 py-2 text-sm text-[var(--pb-text-primary)] placeholder-[var(--pb-input-placeholder)] focus:outline-none focus:border-[var(--pb-input-border-focus)] focus:ring-1 focus:ring-[var(--pb-input-border-focus)] transition-colors" />
          <label className="block text-sm font-medium text-[var(--pb-text-secondary)]">Subheadline</label>
          <input type="text" value={data.subheadline || ""} onChange={(e) => onChange("subheadline", e.target.value || undefined)}
            placeholder="Testimonials from clients and colleagues" className="w-full bg-[var(--pb-input-bg)] border border-[var(--pb-input-border)] rounded-lg px-3 py-2 text-sm text-[var(--pb-text-primary)] placeholder-[var(--pb-input-placeholder)] focus:outline-none focus:border-[var(--pb-input-border-focus)] focus:ring-1 focus:ring-[var(--pb-input-border-focus)] transition-colors" />
        </div>
      </div>
      ...
      <div className={sectionClass}>
        ...
        <SelectField label="Horizontal Alignment" id="alignment" value={data.alignment}
          onChange={(value) => onChange("alignment", value as "left" | "center" | "right")} options={ALIGNMENT_OPTIONS} />
      </div>
      ...
      <div className={sectionClass}>
        ...
        <SelectField label="Card spacing" id="gap" value={data.gap}
          onChange={(value) => onChange("gap", value as "small" | "medium" | "large")} options={GAP_OPTIONS} />
      </div>

      <div className={sectionClass}>
        <h3 className={sectionTitleClass}>Sort Order</h3>
        <SelectField label="Sort testimonials by" id="sortBy" value={data.filters._sortBy || "default"}
          onChange={(value) => {
            onChange("filters", {
              ...data.filters,
              _sortBy: value === "default" ? undefined : (value as TestimonialsData["filters"]["_sortBy"]),
            });
