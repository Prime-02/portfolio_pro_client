...
const Ballpit = ({ className = "", followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    // React 18 Strict Mode double-invokes effects (mount -> unmount -> remount).
    // On the remount the same canvas element comes back but its WebGL context
    // was killed by forceContextLoss() in cleanup. getContext() returns null on
    // a lost context, which causes Three.js to crash reading `.precision`.
    // Probing first lets us bail on the poisoned cycle; the next real mount
    // gets a fresh canvas with a live context.
    const probe = canvas.getContext("webgl2") || canvas.getContext("webgl");
    if (!probe) return;

    let disposed = false;

    // Still defer one rAF so the canvas is fully painted before Three.js
    // reads its dimensions and sets up the renderer.
