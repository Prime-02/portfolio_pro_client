...
import { Crown, Moon, Sun, Zap } from "lucide-react";
...
const Pro = () => {
  ...
  const uploadTheme = async () => {
    ...
    try {
      const { data: uploadRes } = await api.post("/themes", themeData);
      if (uploadRes) {
        toast.success(
