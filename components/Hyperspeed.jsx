  ...
  useEffect(() => {
    ...
    class App {
      ...
      async init() {
        ...
        // Append canvas to DOM BEFORE creating composer
        this.container.append(this.renderer.domElement);
        // Wait for the composer/passes to actually be created before doing
        // anything that depends on them (including starting the tick loop).
        await this.initPasses();
        if (this.disposed) {
          return;
        }
    ...
    const appContainer = hyperspeed.current;
    ...
    // FIX: Type-safe distortion lookup with fallback
    const distortionKey = options.distortion;
    const selectedDistortion = distortions[distortionKey];
    if (!selectedDistortion) {
      console.warn(
        `Unknown distortion "${distortionKey}", falling back to turbulentDistortion`,
      );
      options.distortion = distortions["turbulentDistortion"];
    } else {
      options.distortion = selectedDistortion;
    }

    const myApp = new App(appContainer, options);
    ...
    return () => {
      ...
      // Also clear container on unmount
