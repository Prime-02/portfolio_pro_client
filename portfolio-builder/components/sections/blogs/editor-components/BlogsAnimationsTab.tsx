// portfolio-builder/components/sections/blogs/editor-components/BlogsAnimationsTab.tsx
...
interface BlogsAnimationsTabProps {
  data: BlogsData;
...
export default function BlogsAnimationsTab({ data, onUpdate }: BlogsAnimationsTabProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className={sectionClass}>
        ...
        <SelectField label="Preset" id="animPreset" value={preset} onChange={(v) => onUpdate({ preset: v as BioAnimations["preset"] })} options={PRESET_OPTIONS} />
        {isAnimated && (
          <>
      ...
      {isAnimated && (
        <div className={sectionClass}>
          ...
          <Toggle label="Trigger on scroll" description="Animation plays when section enters viewport" checked={scrollTrigger} onChange={(v) => onUpdate({ scrollTrigger: v })} />
          {scrollTrigger && (
            <Toggle label="Animate only once" description="Won't replay when scrolling back up" checked={scrollOnce} onChange={(v) => onUpdate({ scrollOnce: v })} />
          )}
        </div>
      )}
      ...
      <div className={sectionClass}>
        ...
        <SelectField label="Hover behavior" id="hoverEffect" value={hoverEffect} onChange={(v) => onUpdate({ hoverEffect: v as BioAnimations["hoverEffect"] })} options={[
          { value: "none", label: "None" },
