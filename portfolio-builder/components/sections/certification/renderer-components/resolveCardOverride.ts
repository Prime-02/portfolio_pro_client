...
import type {
  CertificationData,
  CertificationCardOverride,
} from "@/portfolio-builder/types/certification";
...
export function resolveCardOverride(
  ...
  for (const override of overrides ?? []) {
    ...
    if (target.issuing_organizations?.includes(cert.issuing_organization || ""))
      matches = true;
    if (target.certification_names?.includes(cert.certification_name || ""))
      matches = true;
    ...
    if (matches) {
      return {
        ...
        showCertificationName:
          override.showCertificationName ?? defaults.showCertificationName,
        showIssuingOrganization:
          override.showIssuingOrganization ?? defaults.showIssuingOrganization,
        showIssueDate: override.showIssueDate ?? defaults.showIssueDate,
        showExpirationDate:
          override.showExpirationDate ?? defaults.showExpirationDate,
        showCertificateLink:
          override.showCertificateLink ?? defaults.showCertificateLink,
        showVerificationBadge:
          override.showVerificationBadge ?? defaults.showVerificationBadge,
        showValidityIndicator:
          override.showValidityIndicator ?? defaults.showValidityIndicator,
