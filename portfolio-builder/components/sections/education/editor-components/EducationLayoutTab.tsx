// portfolio-builder/components/sections/education/editor-components/EducationLayoutTab.tsx

import { EducationData, EducationLayout } from "@/portfolio-builder/types/education";
...
const LAYOUT_OPTIONS: {
  ...
  preview: React.ReactNode;
    ...
    {
      value: "scholarly-list",
      label: "Scholarly List",
    ...
    {
      value: "credentials-carousel",
      label: "Credentials Carousel",
    ...
    {
      value: "transcript-accordion",
      label: "Transcript Accordion",
      description: "Collapsible education entries",
      preview: (
        <svg viewBox="0 0 48 32" fill="none" className="w-full h-full">
          <rect x="4" y="2" width="40" height="6" rx="1" fill="var(--pb-foreground)" opacity="0.2" stroke="var(--pb-foreground)" strokeWidth="0.5" strokeOpacity="0.4" />
          <rect x="4" y="10" width="40" height="6" rx="1" fill="var(--pb-foreground)" opacity="0.15" stroke="var(--pb-foreground)" strokeWidth="0.5" strokeOpacity="0.3" />
          <rect x="4" y="18" width="40" height="6" rx="1" fill="var(--pb-foreground)" opacity="0.15" stroke="var(--pb-foreground)" strokeWidth="0.5" strokeOpacity="0.3" />
          <rect x="4" y="26" width="40" height="4" rx="1" fill="var(--pb-foreground)" opacity="0.1" stroke="var(--pb-foreground)" strokeWidth="0.5" strokeOpacity="0.2" />
          <path d="M40 5l3 2-3 2" stroke="var(--pb-foreground)" strokeWidth="0.5" opacity="0.5" />
        </svg>
      ),
    },
    {
      value: "honors-wall",
      label: "Honors Wall",
      description: "Masonry-style wall of achievements",
      preview: (
        <svg viewBox="0 0 48 32" fill="none" className="w-full h-full">
...
const SORT_OPTIONS = [
  ...
  { value: "institution-asc", label: "Institution (A → Z)" },
  { value: "institution-desc", label: "Institution (Z → A)" },
  { value: "degree-asc", label: "Degree (A → Z)" },
  { value: "degree-desc", label: "Degree (Z → A)" },
];

// Layouts where a column count does not apply
const SINGLE_COLUMN_LAYOUTS: EducationLayout[] = ["scholarly-list", "academic-timeline", "transcript-accordion"];

export default function EducationLayoutTab({ data, onChange }: EducationLayoutTabProps) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className={sectionClass}>
        ...
        <div className="space-y-3">
          ...
          <input
            ...
            placeholder="My academic journey"
            className="w-full bg-[var(--pb-input-bg)] border border-[var(--pb-input-border)] rounded-lg px-3 py-2 text-sm text-[var(--pb-text-primary)] placeholder-[var(--pb-input-placeholder)] focus:outline-none focus:border-[var(--pb-input-border-focus)] focus:ring-1 focus:ring-[var(--pb-input-border-focus)] transition-colors"
          />
        </div>
      </div>
      ...
      {!SINGLE_COLUMN_LAYOUTS.includes(data.layout) && (
        <div className={sectionClass}>
          <h3 className={sectionTitleClass}>
            {data.layout === "credentials-carousel" ? "Cards per Page" : "Columns"}
          </h3>
          <SliderField
            label={data.layout === "credentials-carousel" ? "Cards per page" : "Number of columns"}
            value={data.columns}
            min={1}
            max={4}
            step={1}
      ...
      <div className={sectionClass}>
        ...
        <SelectField
          label="Sort educations by"
          id="sortBy"
          value={data.filters._sortBy || "default"}
          onChange={(value) => {
            onChange("filters", {
              ...data.filters,
              _sortBy: value === "default" ? undefined : (value as EducationData["filters"]["_sortBy"]),
            });
          }}
