...
function UrlPicker({
    ...
    // Track the selected mode separately from the actual URL value
    const [mode, setMode] = useState<"internal" | "external">(() => {
        // Initialize based on whether current value matches an internal option
    ...
    // The dropdown value: either the matching internal option id, or the external sentinel
    const dropdownValue = mode === "internal" && isInternalUrl
        ? value
        : mode === "external"
            ? EXTERNAL_LINK_ID
            : "";
            ...
            <Dropdown
                ...
                onSelect={(val) => {
                    if (val === EXTERNAL_LINK_ID) {
                        setMode("external");
                        onChange(""); // Clear any previous internal URL
                    } else {
                        setMode("internal");
                    ...
                    <div key={index} className="border border-[var(--pb-border)] rounded-lg overflow-hidden bg-[var(--pb-card-bg)]">
                        ...
                        {!isCollapsed && (
                            <div className="p-4 space-y-4">
                                ...
                                {/* ── URL Picker (replaces old Textinput) ── */}
                                <UrlPicker
                                    value={btn.url}
                                    username={userInfo?.username || ""}
                                    onChange={(newUrl) => onUpdate(index, { url: newUrl })}
                                />
